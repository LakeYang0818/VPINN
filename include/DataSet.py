...
class DataSet:
    ...
    def __init__(
        ...
        *,
        ...
        if isinstance(coords, torch.Tensor):
            if not as_tensor:
                warnings.warn(
                    f"You have passed {type(coords)} arguments but set 'as_tensor = False'. "
        ...
        if not as_tensor:
            ...
            if isinstance(coords, torch.Tensor):
                ...
                self._coords = torch.stack(
                    [
                        torch.tensor(val, dtype=dtype, requires_grad=requires_grad)
                        for val in coords
                    ]
                )
    ...
    def __str__(self) -> str:
        return (
            f"Dataset of {len(self._coords)} {len(self._data[0])}-dimensional function values on "
