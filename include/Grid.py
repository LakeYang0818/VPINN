...
class Grid:
    def __init__(
        ...
        *,
        ...
        if len(x) == 0:
            raise ValueError(
                f"Invalid arg 'x' for grid: cannot generate grid from empty {type(x)}."
            )
        else:
            self._n_x = len(x)
        ...
        if isinstance(x, torch.Tensor):
            if x.dim() > 2:
                raise ValueError(
                    f"Invalid arg 'x' for grid: cannot generate grid from {x.dim()}-dimensional tensor."
                )
            elif not as_tensor:
                warnings.warn(
                    f"You have passed a {type(x)} argument for 'x' but set 'as_tensor = False'. "
        ...
        if requires_grad and not as_tensor:
            warnings.warn(
                "You have set 'as_tensor' to False but 'requires_grad' to 'True'. 'requires_grad' is only"
        ...
        if y is not None:
            if isinstance(y, torch.Tensor):
                if y.size() == torch.Size([0]):
                    raise ValueError(
                        f"Invalid arg 'y' for grid: cannot generate grid from empty tensor."
                    )
                elif y.dim() > 2:
                    raise ValueError(
                    ...
                    warnings.warn(
                        f"You have passed a {type(y)} argument for 'y' but set 'as_tensor = False'. "
                        f"In this case, 'as_tensor' will be set to true."
                    )
                    as_tensor = True
                self._n_y = y.size()[0]
                ...
                if len(y) == 0:
                    raise ValueError(
                        f"Invalid arg 'y' for grid: cannot generate grid from empty {type(y)}."
                    )
                else:
                    self._n_y = len(y)
            self._dim = 2
        ...
        if not as_tensor:
            ...
            self._y = (
                np.resize(np.array(y).astype(dtype), (self._n_y, 1))
                if y is not None
                else np.array([])
            )
            ...
            if isinstance(x, torch.Tensor):
                self._x = torch.reshape(
                    torch.tensor(x.detach().clone().numpy(), dtype=dtype),
                    (self._n_x, 1),
                )
            else:
            ...
            if y is not None:
                if isinstance(y, torch.Tensor):
                    self._y = torch.reshape(
                        torch.tensor(y.detach().clone().numpy(), dtype=dtype),
                        (self._n_y, 1),
                    )
                else:
                    self._y = torch.reshape(
                        torch.tensor(y, dtype=dtype), (self._n_y, 1)
                    )
        ...
        if y is None:
            if not as_tensor:
                ...
                self._data = self._x
                self._boundary = torch.reshape(
                    torch.stack([self._x[0], self._x[-1]]), (2, 1)
                )
            ...
            for j in range(self._n_y):
                for i in range(self._n_x):
                    if not as_tensor:
                        ...
                        point = torch.reshape(
                            torch.stack([self._x[i], self._y[j]]), (1, 2)
                        )
                    data.append(point)
            ...
            for i in range(self._n_x):
                if not as_tensor:
                    ...
                    lower.append(
                        torch.reshape(torch.stack([self._x[i], self._y[0]]), (1, 2))
                    )
                    upper.append(
                        torch.reshape(
                            torch.stack([self._x[len(self._x) - i - 1], self._y[-1]]),
                            (1, 2),
                        )
                    )
            for j in range(self._n_y):
                if not as_tensor:
                    ...
                    right.append(
                        torch.reshape(torch.stack([self._x[-1], self._y[j]]), (1, 2))
                    )
                    left.append(
                        torch.reshape(
                            torch.stack([self._x[0], self._y[len(self._y) - j - 1]]),
                            (1, 2),
                        )
                    )
            ...
            if not as_tensor:
                ...
                self._boundary = np.resize(
                    np.array(boundary), (2 * (self._n_x + self._n_y - 2), 2)
                )
                ...
                self._interior = (
                    np.resize(
                        np.array(interior), ((self._n_x - 2) * (self._n_y - 2), 2)
                    )
                    if interior != []
                    else []
                )
            else:
                self._data = torch.reshape(
                    torch.stack(data), (self._n_x * self._n_y, 2)
                )
                self._boundary = torch.reshape(
                    torch.stack(boundary), (2 * (self._n_x + self._n_y - 2), 2)
                )
                ...
                self._interior = (
                    torch.reshape(
                        torch.stack(interior), ((self._n_x - 2) * (self._n_y - 2), 2)
                    )
                    if interior != []
                    else []
                )
...
def construct_grid(
    *,
    ...
    if dim == 1:
        ...
        return Grid(
            x=_grid_1d(boundary[0], boundary[1], grid_size),
            as_tensor=as_tensor,
            dtype=dtype,
...
def rescale_grid(grid: Grid, *, new_domain) -> Grid:
    ...
    if grid.dim == 1:
        if np.shape(new_domain) != (2,):
            raise ValueError(
                f"Cannot rescale 1d grid to {len(np.shape(new_domain))}-dimensional domain!"
            )
        ...
        return Grid(
            ...
            * (grid.x - grid.x[0])
            ...
            requires_normals=(grid.normals is not None),
        )

    elif grid.dim == 2:
        if np.shape(new_domain) != (2, 2):
            raise ValueError(
                f"Cannot rescale 2d grid to {len(np.shape(new_domain))}-dimensional domain!"
            )

        return Grid(
            x=(new_domain[0][1] - new_domain[0][0])
            * (grid.x - grid.x[0])
            / (grid.x[-1] - grid.x[0])
            + new_domain[0][0],
            y=(new_domain[1][1] - new_domain[1][0])
            * (grid.y - grid.y[0])
            / (grid.y[-1] - grid.y[0])
            + new_domain[1][0],
            as_tensor=grid.is_tensor,
...
def construct_grid_from_cfg(
    cfg: dict,
    *,
    as_tensor: bool = True,
    requires_grad: bool = False,
    requires_normals: bool = True,
) -> Grid:
    ...
    grid_boundary: Sequence = (
        cfg["boundary"]["x"]
        if dim == 1
        else [cfg["boundary"]["x"], cfg["boundary"]["y"]]
    )
