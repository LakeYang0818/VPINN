...
def evaluate_test_functions_on_grid(grid: xr.DataArray,
    ...
    for j, idx in enumerate(test_function_indices.data.reshape((-1, grid.attrs['grid_dimension']))):
        ...
        ds = xr.apply_ufunc(partial(test_function,
                            ...
                            grid, input_core_dims=[['idx']], dask='allowed', vectorize=True)
    ...
    # Add attributes
    space_dims = list(grid.coords)
    space_dims.remove('idx')
    res.attrs['test_function_dims'] = tf_labels
    res.attrs['space_dimensions'] = space_dims
    res.attrs['grid_dimension'] = grid.attrs['grid_dimension']
    res.attrs['grid_density'] = grid.attrs['grid_density']
...
# # @adapt_input
# def test_function(point: Any, n: int, *, which: str = 'Legendre') -> float:
...
#     """
#     if n < 1:
#         raise ValueError(f'Invalid test function index {n}; expected value greater than 0!')
#
#     if which == "Chebyshev":
...
#
#         if isinstance(point, numbers.Number):
#             return jacobi_poly(point, n + 1, a=0, b=0) - jacobi_poly(point, n - 1, a=0, b=0)
#         else:
#             res = 1
#             for coord in point:
...
#                                         data=[dtest_function(domain_x, idx[0], d=0, which=which) for idx in indices.x],
...
#                                         data=[dtest_function(domain_x, idx[0], d=d, which=which) for idx in indices.x],
#                                         as_tensor=grid.is_tensor, dtype=grid.dtype, requires_grad=grid.requires_grad)
#
...
#                                         data=[dtest_function(domain_y, idx[0], d=d, which=which) for idx in indices.y],
#                                         as_tensor=grid.is_tensor, dtype=grid.dtype, requires_grad=grid.requires_grad)
#
...
#                                                           y=vals_y_d.data[indices.data[i][1] - 1]).data, dim=1,
#                                                      keepdim=True)
#                                           for i in range(n_test_funcs)])
#
...
#                             1)
#                         left = np.stack(
#                             (test_function(grid.y[1:-1][::-1], k[1], which=which).flatten() * x_0,
#                              np.zeros(len(grid.y[1:-1]))),
#                             1)
#                     else:
#                         lower = torch.stack(
#                             [torch.zeros(len(grid.x)),
#                              torch.flatten(test_function(grid.x, k[0], which=which)) * y_0],
#                             dim=1)
#                         right = torch.stack(
#                             [torch.flatten(test_function(grid.y[1:-1], k[1], which=which)) * x_1,
#                              torch.zeros(len(grid.y[1:-1]))],
#                             dim=1)
#                         # Note: torch currently does not support the [::-1] syntax that can be used with numpy arrays.
#                         upper = torch.stack(
#                             [torch.zeros(len(grid.x)),
#                              torch.flatten(test_function(grid.x.flip(0), k[0], which=which)) * y_1],
#                             dim=1)
#                         left = torch.stack(
#                             [torch.flatten(test_function(grid.y[1:-1].flip(0), k[1], which=which)) * x_0,
#                              torch.zeros(len(grid.y[1:-1]))],
#                             dim=1)
#
