...
def validate_cfg(cfg: dict):
    ...
    if dim not in {1, 2}:
        raise ValueError(
            f"Argument {dim} not supported! Dimension must be either 1 or 2!"
        )
    if dim == 1:
        if cfg["PDE"]["type"] in ["Burger", "PorousMedium"]:
            raise TypeError(
                f"The {cfg['PDE']['type']} equation requires a two-dimensional grid!"
            )
        ...
        if cfg["space"]["grid_size"]["x"] < 3:
            raise ValueError(
                "Grid size in dimension 1should be at least 2! Increase grid size."
            )
...
def adapt_input(func=None, *, dtype=torch.float, requires_grad=False):
    ...
    def evaluate(x: Any, *args, **kwargs) -> Any:
        ...
        if isinstance(x, torch.Tensor):
            ...
            if x.dim() > 2:
                raise ValueError(
                    f"Cannot evaluate functions on {x.dim()}-dimensional tensor!"
                )
            ...
            # If x is a scalar
                ...
                return torch.reshape(
                    torch.tensor([res], dtype=dtype, requires_grad=requires_grad), (1,)
                )
            ...
            # If x is a single point:
                ...
                res = func(x.detach().clone().numpy(), *args, **kwargs)
                return torch.reshape(
                    torch.tensor([res], dtype=dtype, requires_grad=requires_grad),
            ...
            # If x is a list of points
                ...
                return torch.reshape(
                    torch.stack(res), (int(len(res)), int(np.prod(np.shape(res[0]))))
                )
        ...
        # Evaluate on sequences that are not torch.Tensors
            ...
            if len(np.shape(x)) > 2:
                raise ValueError(
                    f"Cannot evaluate functions on {len(np.shape(x))}-dimensional sequence!"
                )
    ...
    if not as_tensor:
        ...
        if len(func_1.size()) == 1 or func_1.size()[-1] == 1:
            ...
            res = (
                domain_volume
                / len(func_2)
                * torch.einsum("i..., i...->...", func_1, func_2)
            )
