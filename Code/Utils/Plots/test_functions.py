...
from ..Datatypes import Grid, rescale_grid


"""Plots the test functions and their derivatives up to order n"""


def plot_test_functions(grid: Grid, *, order: int, d: int = 1, show: bool = False, which: str):
    ...
    if grid.dim == 1:
        ...
        for i in range(order):
            ...
            axs[i].plot(grid.data, test_function(grid.data, i+1, which=which), color='peru', label=fr'$v_{str(i)}(x)$')
            if d > 0:
                label_derivs = fr'$v^\prime_{str(i)}(x)$' if d==1 else fr'$d^{str(d)}v_{str(i)}(x)$'
                axs[i].plot(grid.data, dtest_function(grid.data, i+1, d=d, which=which), color='darkred', label=label_derivs)
            axs[i].legend(ncol=2)
        ...
        for i in range(order):
            img = axs[i].imshow(torch.reshape(torch.prod(dtest_function(grid.data, i+1, d=d, which=which), dim=1),
                        (len(grid.x), len(grid.y))),
