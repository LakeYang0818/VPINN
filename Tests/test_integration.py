import numpy as np
import torch
from Utils import Grid, integrate
...
def test_1d_integration():
    ...
    to_test = [
        ...
        {
            "space": Grid(
                x=np.linspace(0, 2 * np.pi, 400), as_tensor=False, dtype=np.float64
            ),
        ...
        {
            "space": Grid(
                x=np.linspace(0, 2 * np.pi, 100000), as_tensor=False, dtype=np.float64
            ),
        ...
        {
            "space": Grid(
                x=np.linspace(-3, 3, 1000000), as_tensor=True, requires_grad=False
            ),
    ...
    for test_obj in to_test:
        integral = integrate(
            test_obj["function"](test_obj["space"]),
        ...
        if not test_obj["space"].is_tensor:
            ...
            np.testing.assert_allclose(
                integral.detach().numpy(), test_obj["result"], atol=1e-4
            )
...
def test_2d_integration():
    ...
    to_test = [
        ...
        },
        {
            ...
            "function": lambda x: torch.stack(
                [torch.sin(p[0]) * torch.sin(p[1]) for p in x.data]
            ),
            "result": 0.0,
            "tolerance": 1e-7,
        },
    ...
    for test_obj in to_test:
        ...
        if not test_obj["space"].is_tensor:
            np.testing.assert_allclose(
                integral, test_obj["result"], atol=test_obj["tolerance"]
            )
        else:
            np.testing.assert_allclose(
                integral.detach().numpy(),
                test_obj["result"],
                atol=test_obj["tolerance"],
            )
...
def test_vector_integration():
    ...
    np.testing.assert_allclose(
        integral1, 8 / 3 * grid1.volume**2 / (2**4), atol=1e-1
    )
