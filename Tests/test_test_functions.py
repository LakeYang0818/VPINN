...
from Utils import Grid, construct_grid, dtest_function, rescale_grid
from Utils import test_function as tfunc
from Utils import testfunc_grid_evaluation as tfunc_eval
...
def check_test_function(type: str, index: int):
    ...
    assert tfunc(torch.tensor([[-1, -1, -1], [-1, 1, 1]]), index, which=type)[
        0
    ] == torch.tensor([0])
    assert tfunc(torch.tensor([[-1, -1, -1], [-1, 1, 1]]), index, which=type)[
        1
...
def check_test_function_derivatives(type: str, index: int, d: int):
    ...
    if d == 0:
        assert (
            dtest_function(test_array, index, d=d, which=type)
            == tfunc(test_array, index, which=type)
        ).all()
    ...
    if type in ["Legendre", "Chebyshev"]:
        if d == index + 1:
            if type == "Chebyshev":
                assert (
                    dtest_function(test_array, index, d=d, which=type)
                    == np.ones(5) * gamma(index + 2) * 2**index
                ).all()
            elif type == "Legendre":
                assert (
                    dtest_function(test_array, index, d=d, which=type)
        ...
        if d > index + 1:
            assert (
                dtest_function(test_array, index, d=d, which=type) == [0, 0, 0, 0, 0]
            ).all()
...
def test_evaluation_on_grid():
    domains = {
        "all": lambda x: x.data,
        "boundary": lambda x: x.boundary,
        "interior": lambda x: x.interior,
    }
        ...
        for grid in grids1d:
            for order in d:
                ...
                if not grid.is_tensor:
                    assert (
                        vals.coords.flatten() == np.arange(1, 11, 1, dtype=float)
                    ).all()
                    assert len(np.shape(vals.data)) == 3
                    assert np.shape(vals.data) == (10, len(value(grid)), 1)
                else:
                    (
                        vals.coords.detach().numpy() == np.arange(1, 11, 1, dtype=float)
                    ).all()
                ...
                for i in range(len(vals.data)):
                    assert (
                        vals.data[i]
                        == dtest_function(
                            value(grid_rescaled), i + 1, d=order, which="Legendre"
                        )
                    ).all()
                ...
                if order > 0:
                    for k in range(len(vals.data)):
                        for p in range(len(value(grid))):
                            ...
                            test_val = [
                                dtest_function(
                                    value(grid_rescaled).data[p][0],
                                    int(vals.coords[k][0].detach().numpy()),
                                    d=order,
                                    which="Chebyshev",
                                )
                                * dtest_function(
                                    value(grid_rescaled).data[p][1],
                                    int(vals.coords[k][1].detach().numpy()),
                                    d=0,
                                    which="Chebyshev",
                                ),
                                dtest_function(
