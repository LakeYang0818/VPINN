import numpy as np
import torch
from Utils import DataSet, Grid, dtest_function
...
def test_DataSet_initialization_1d():
    ...
    for coord in coords:
        ...
        as_tensor = isinstance(coord, torch.Tensor) or (
            isinstance(coord, Grid) and Grid.is_tensor
        )
        test_dataset = DataSet(
            coords=coord, data=func_vals, dtype=type(coord), as_tensor=as_tensor
        )
...
def test_DataSet_initialization_2d():
    ...
    for coord in coords:
        ...
        test_dataset = DataSet(
            coords=coord, data=func_vals, dtype=np.float64, as_tensor=as_tensor
        )
        assert test_dataset.dim == 2
        assert (test_dataset.coords == coord).all()
        assert (test_dataset.data == func_vals).all()
