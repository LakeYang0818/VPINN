...
def check_test_function(type: str, index: int):
    ...
    for input in expect_sequence_outputs:
        val = tfunc(input, index, which=type)
        if isinstance(val, torch.Tensor):
            assert (val.size() == torch.Size([len(val)]))
            assert (val.dim() == 1)
            assert (isinstance(val, torch.Tensor))
        elif isinstance(val, Grid):
            assert (np.shape(val) == val.size)
        else:
            assert (len(np.shape(val)) == 1)
            assert np.shape(val) == (len(val),)
            assert (not isinstance(val, torch.Tensor))
...
def check_test_function_derivatives(type: str, index: int, d: int):
    inputs = [
        # 0.2, does not work: output shape != input shape
        [0.3],
        [0.3, 0.6],
        [[0.4, -2], [0.6, 0.8]],
        torch.tensor([0.3]),
    ...
    if d == 0:
        assert (dtest_function(test_array, index, d=d, which=type).flatten()
                == tfunc(test_array, index, which=type)).all()
                    ...
                    for k in range(len(vals.data)):
                        for p in range(len(value(grid))):
                            ...
                            test_val = [
                                ...
                                dtest_function(value(grid_rescaled).data[p][1], int(vals.coords[k][1].detach().numpy()),
                                               d=order, which='Chebyshev')]
                            test_val = torch.reshape(torch.stack(test_val), (1, 2))
