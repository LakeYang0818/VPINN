...
def loss(model, t_interior, X_interior, t_terminal, X_terminal):
    ...
    V_x = tf.gradients(ys=V, xs=X_interior)[0]
    V_xx = tf.gradients(ys=V_x, xs=X_interior)[0]
    ...
    L3 = tf.reduce_mean( input_tensor=tf.square(fitted_terminal - target_terminal) )
...
def compute_fitted_optimal_control(V):
    V_x = tf.gradients(ys=V, xs=X_interior_tnsr)[0]
    V_xx = tf.gradients(ys=V_x, xs=X_interior_tnsr)[0]
