...
# TF v1 to TF v2 compatibility issue: tf.placeholder() is not compatible with eager execution
...
def loss(model, t, x_interior, x_initial, nSim_t, alpha, beta):
    ...
    for tIndex in range(nSim_t):
        ...
        u_t  = tf.gradients(ys=u, xs=t_vector)[0]
        ...
        psi_denominator = tf.reduce_sum(input_tensor=tf.exp(-u))
        psi = tf.reduce_sum( input_tensor=u_t*tf.exp(-u) ) / psi_denominator
        ...
        currLoss = tf.reduce_mean(input_tensor=tf.square(diff_f))
    ...
    L3 = tf.reduce_mean(input_tensor=tf.square(fitted_pdf - target_pdf))
...
for i, curr_t in enumerate(densityTimes):
    ...
    plt.hist(sim_x, bins=40, density=True, stacked=True, color = 'b')
